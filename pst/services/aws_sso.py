...
class AwsSsoService:
    ...
    def authenticate(self, profile_name: str):
        ...
        try:
            ...
            if isinstance(e, UnauthorizedSSOTokenError) or "UnauthorizedException" in str(e):
    ...
    def _is_authenticated(self, profile_name: str):
        try:
            ...
            if "ExpiredToken" in str(e):
                print(f"SSO token is expired for profile: {profile_name}")
                return False
            else:
                print(f"Failed to check SSO authentication status: {e}")
                raise
            return False
